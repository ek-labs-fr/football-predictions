...
| **Cloudflare Pages** for hosting | See `documents/cloudflare-pages-migration-plan.md` for the full rationale. Short version: free TLS, free custom domain, GitHub-connected CI/CD, edge cache, and a Pages Function as the `/data/*` proxy. The original AWS CloudFront-fronted plan got blocked on AWS's verification queue and was decommissioned. |
